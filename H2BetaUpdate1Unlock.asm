...
%define HacksSegmentAddress             007f6000h           ; Virtual address of the .hacks segment
...
%define MenuHandler_MainMenu                            001AF3F6h
%define MenuHandler_GamertagSelect                      002A3D13h

%define Create_MainMenu_Campaign                        002A4138h
%define Create_XboxLive_Menu                            001AF454h
%define Create_MainMenu_XboxLive                        002A4115h
%define Create_MainMenu_SplitScreen                     002A40CFh
%define Create_MainMenu_SystemLink                      002A40F2h
%define Create_MainMenu_OptionsMenu                     001AF1B6h

%define LoadScreen                                      000D68C9h
...
%define PrintDebugMessage                               000AC0D0h

%define CreateNetworkSquadBrowserScreen                 001B2460h

; Kernel imports:
%define imp_DbgPrint                                    00470D8Ch

%define g_network_link                                  006DB050h
%define broadcast_search_globals_message_gateway        006D9980h

%define c_network_message_gateway__send_message         002680A0h
%define _broadcast_search_globals_get_session_nonce     00284AE0h
%define get_transport_address                           002FF0B0h         ; Very different in TU1, probably TU2 as well.

%define setsockopt                                      00440254h
%define GetLastError                                    0031ACA9h
%define malloc                                          0038F55Eh
%define free                                            00391BDDh
...
;-------------------------------------------------------------
    ...
    ;---------------------------------------------------------      
    dd          (001A81B0h - ExecutableBaseAddress)
    ...
    ;---------------------------------------------------------      
    dd          (001AF112h - ExecutableBaseAddress)
    dd          (_i_dont_agree_end - _i_dont_agree_start)
    _i_dont_agree_start:

            ; Skip the call.
            times (001AF119h - 001AF112h) db 90h
    ...
    ;---------------------------------------------------------
    dd          (0x190F0A - ExecutableBaseAddress) + 0x12
    ...
    ;---------------------------------------------------------
    dd          (00284B8Ch - ExecutableBaseAddress)
    ...
    ;---------------------------------------------------------
    dd          (00279D4Eh - ExecutableBaseAddress)
    ...
    ;---------------------------------------------------------
    dd          (0x30288A - ExecutableBaseAddress)
    ...
    AddGameVariantMenuOptions_mid:
            times (0x3028D3 - (0x30288A + (AddGameVariantMenuOptions_mid - AddGameVariantMenuOptions_start))) db 0x90
...
%if ENABLE_DEBUG_OUTPUT != 0
    ...
    ; Patch log level
    ;---------------------------------------------------------      
    dd          (0014D50Fh - ExecutableBaseAddress)
    ...
    ;---------------------------------------------------------      
    dd          (0014CFD5h - ExecutableBaseAddress)
...
;-------------------------------------------------------------
    ...
    ;---------------------------------------------------------
    dd            (0x149C68 - ExecutableBaseAddress)
    ...
    ;---------------------------------------------------------
    dd            (00149A70h - ExecutableBaseAddress)                      ; Very different in this TU. May need to be edited or removed.
    ...
    ;---------------------------------------------------------
    dd            (001C1C50h - ExecutableBaseAddress)
    ...
    ;---------------------------------------------------------
    ; Stop clients from crashing when negotiating new host                                 ; game_results_get_game_recording Does not exist in TU1 or TU2. 
    ;---------------------------------------------------------
...
;---------------------------------------------------------
    ...
    _Hack_NetworkSquadListUpdate_Hook_done:
            ...
            mov     al, [006DB16Ch]     ; Check which tick count to use
            test    al, al
